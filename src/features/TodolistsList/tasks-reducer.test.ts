...
test("correct task should be added to correct array", () => {
  ...
  expect(endState["todolistId2"][0].status).toBe(TaskStatuses.Active);
});
...
test("status of specified task should be changed", () => {
  const action = updateTask({
    taskId: "2",
    todolistId: "todolistId2",
    model: { status: TaskStatuses.Active },
  });

  const endState = tasksReducer(startState, action);

  expect(endState["todolistId1"][1].status).toBe(TaskStatuses.Completed);
  expect(endState["todolistId2"][1].status).toBe(TaskStatuses.Active);
});
...
test("title of specified task should be changed", () => {
  ...
  expect(endState["todolistId2"][1].title).toBe("yoghurt");
